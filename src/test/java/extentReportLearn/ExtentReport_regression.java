...
public class ExtentReport_regression {
    ...
    public void createSetup(){
        ...
        spark = new ExtentSparkReporter(System.getProperty("user.dir")+"/ExtentReport/reqresTestExecutionReport_regression.html"); //current path of the project using System.getProperty
//System.getProperty("user.dir") == C:\Users\sunilk\IdeaProjects\TestNG_RestAssurd\ExtentReport
    ...
    public void validateSoapIdAndEmail() {
        logger.info("This is for regression test information about last_name validation");
        logger.info("Get API Url is: "+"https://reqres.in/api/users?page=2");
        Response response = get("https://reqres.in/api/users?page=2");
        String res = response.getBody().asString();

        //first get count of node you want to test
        int jsonPathCount = response.jsonPath().getList("data.last_name").size();
        ...
        for (int i = 0; i < jsonPathCount; i++) {
            ...
            String last_name = response.getBody().jsonPath().getString("data.last_name[" + i + "]");

            System.out.println("If ID is ==>" + id + " " + "Then last_name value is ==> " + last_name);
            logger.info("If ID is ==>" + id + " " + "Then last_name value is ==> " + last_name);
